...
#[SkipMenu]
class CorruptionRiskPositionPage extends Page
{
    protected ?string $layout = GuestLayout::class;

    public function getTitle(): string
    {
        return 'Должности, подверженные коррупционным рискам';
    }
    ...
    public function getBreadcrumbs(): array
    {
        return [
            '/' => 'Главная',
            '/about/corruption-risk-map/positions' => $this->getTitle()
        ];
    }
    ...
    protected function components(): iterable
    {
        $positions = CorruptionRiskPosition::query()
            ...
            ->get()
            ->each(function (CorruptionRiskPosition $item) {
        ...
        if ($positions->isEmpty()) {
            return [
                Box::make('Должности, подверженные коррупционным рискам', [
        ...
        foreach ($positions as $position) {
            if ($position->file_url) {
                $linksHtml .= '<li style="margin-bottom: 1rem; padding: 0.75rem; border-bottom: 1px solid #dee2e6;">';
                $linksHtml .= '<a href="' . e($position->file_url) . '" target="_blank" style="color: #1977cc; text-decoration: none; font-size: 1rem; font-weight: 500; display: inline-flex; align-items: center; gap: 0.5rem; transition: color 0.2s;" onmouseover="this.style.color=\'#0d5aa7\'" onmouseout="this.style.color=\'#1977cc\'">';
