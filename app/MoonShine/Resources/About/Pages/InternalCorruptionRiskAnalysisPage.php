...
#[SkipMenu]
class InternalCorruptionRiskAnalysisPage extends Page
{
    protected ?string $layout = GuestLayout::class;

    public function getTitle(): string
    {
        return 'Внутренний анализ коррупционных рисков';
    }
    ...
    public function getBreadcrumbs(): array
    {
        return [
            '/' => 'Главная',
            '/about/compliance-service/corruption-risk-analysis' => $this->getTitle()
        ];
    }
    ...
    protected function components(): iterable
    {
        $analyses = InternalCorruptionRiskAnalysis::query()
            ...
            ->get()
            ->each(function (InternalCorruptionRiskAnalysis $item) {
        ...
        if ($analyses->isEmpty()) {
            return [
                Box::make('Внутренний анализ коррупционных рисков', [
        ...
        foreach ($analyses as $analysis) {
            if ($analysis->file_url) {
                ...
                $linksHtml .= '<a href="' . e($analysis->file_url) . '" target="_blank" style="color: #1977cc; text-decoration: none; font-size: 1rem; font-weight: 500; display: inline-flex; align-items: center; gap: 0.5rem; transition: color 0.2s;" onmouseover="this.style.color=\'#0d5aa7\'" onmouseout="this.style.color=\'#1977cc\'">';
