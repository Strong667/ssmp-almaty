...
#[SkipMenu]
class CorruptionRiskListPage extends Page
{
    protected ?string $layout = GuestLayout::class;

    public function getTitle(): string
    {
        return 'Перечень коррупционных рисков';
    }
    ...
    public function getBreadcrumbs(): array
    {
        return [
            '/' => 'Главная',
            '/about/corruption-risk-map/list' => $this->getTitle()
        ];
    }
    ...
    protected function components(): iterable
    {
        $lists = CorruptionRiskList::query()
            ...
            ->get()
            ->each(function (CorruptionRiskList $item) {
        ...
        if ($lists->isEmpty()) {
            return [
                Box::make('Перечень коррупционных рисков', [
        ...
        foreach ($lists as $list) {
            if ($list->file_url) {
                ...
                $linksHtml .= '<a href="' . e($list->file_url) . '" target="_blank" style="color: #1977cc; text-decoration: none; font-size: 1rem; font-weight: 500; display: inline-flex; align-items: center; gap: 0.5rem; transition: color 0.2s;" onmouseover="this.style.color=\'#0d5aa7\'" onmouseout="this.style.color=\'#1977cc\'">';
